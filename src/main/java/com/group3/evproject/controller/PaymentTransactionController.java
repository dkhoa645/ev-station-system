...
public class PaymentTransactionController {
    ...
    public void payCallbackHandler(HttpServletResponse response, HttpServletRequest request) throws IOException {
            ...
            Long id = paymentTransactionService.getCompanyOrUserId(ref);
            if (status.equals("00")) {
                ...
                if (result.equals("bookingSuccess")) {
                    ...
                    response.sendRedirect("http://localhost:5173/payment/invoice/" +id);
                }else if(result.equals("companyPaymentSuccess")) {
                    response.sendRedirect("http://localhost:5173/company/invoice/"  +id);
                }
            } else {
